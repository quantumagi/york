...
Deterministic scripts that produce the key certificates/constants referenced in the paper.
...
- The **TT–sector normalization \( \kappa_\infty \)** obtained by **combining** a signed \(\chi_{TT}\) with the χ-free quantity \(\Delta:=M_4/3-\mathbb{E}[u_x^4]\) (see `kappa.py`). The quantity **\(M_4\)** itself is computed via **stabilized Bessel–Laplace integrals** with a **√X** tail extrapolation (see `m4.py`).
- The **Fejér–York lock** that produces the effective length \(L_{\mathrm{eff}}\), reduced mass \(m_r=e^{L_{\mathrm{eff}}/m}\), and a prediction for
  \( \alpha^{-1} = \frac{\kappa_\infty\, m}{e^{2L_{\mathrm{eff}}/m}-1} \).
- **Explicit error control:** absolutely convergent series/pieces with printed remainder bounds, and bracketed/bisected lock solution.
- **Ablations/diagnostics:** dyadic gate & isolation at the EL lock, parity and A1g factors assembled from small, auditable scripts.

> Scope note: this minimal repro implements the **baseline Bessel route** for
> \(M_4\) (feeding \(\Delta\) into \(\kappa_\infty\)). There is **no separate “Route A” continuum cross-check** in this repo, and no commutator \(z_\chi\) experiments.

## Components (src/)

- `xq.py` — **Fejér quarter-FWHM lock** \(K(x_q)=8/15\) via rigorous bisection; emits `x_q`, `L0=log(15/8)`, and derivatives `fpp`, `f3`, `f4`.  
...
- `m4.py` — **Asymptotic fourth-moment ratio** \(M_4\) via **stabilized Bessel–Laplace integrals** and **√X** tail extrapolation; prints per-round M4 refinements and records the **tail-span width** of recent M4 values.

- `kappa.py` — **TT combiner** for \( \kappa_\infty \): uses **signed** \(\chi_{TT}\) and χ-free \(\Delta\) (from `m4.py` + \(\mathbb{E}[u_x^4]\)) to form \( \kappa_\infty = \tfrac12 - \chi_{TT}^{\text{signed}}\,\Delta \). Propagates an optional \(|\delta\Delta|\) to \(|\delta\kappa|\).
...
- `zdir.py` — **Main-lobe share (K-independent)**:
  \( \zeta_{\text{dir}} = \tfrac{8}{25} \), \( \zeta_{\text{wedge}} = \zeta_{\text{dir}}/N \),
  where `N` is the number of parity sectors (e.g. `5`).
...
# Baseline κ∞ combiner (pulls χ_TT, Δ from deps; Δ uses M4 from m4.py)
...
python src/zdir.py --num-sectors 5

# 4) M4 and κ∞
python src/m4.py --E-u4 1/5
python src/kappa.py --chi-tt 11/260 --E-u4 1/5

# 5) Quadratic correction & final observables
...
- `outputs/m4.json` ⇒ stable \(M_4\) values over the last few refinements and a small reported **tail-span width**.
- `outputs/kappa.json` ⇒ a stable `κ_∞` with optional propagated `dκ` if you supplied `dΔ`.

## How things plug together

- `xq.py` → `x_q`, `L0`, `fpp`, `f3`, `f4`  
- `m.py`  → validates odd-`m` dyadic/isolation at `x_q`  
- Small audited factors: `h4.py`, `cq.py`, `chitt.py`, `j2.py`, `zparity.py`, `edge_parity.py`,
  `num_orbits.py`, `zdir.py`, `a1g_xi_tt.py`, `midedge_blend_ratio_mc.py`  
- `m4.py` → computes \(M_4\) (Bessel–Laplace + √X tail), reports tail-span  
- `gamma.py` → combines parity/Jacobian/direction/A1g into \( \Gamma \)  
- `kappa.py` → combines \(\Delta\) with \(\chi_{TT}\) to get \( \kappa_\infty \)  
