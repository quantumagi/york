...
- The **TT–sector normalization ($\kappa_\infty$)** obtained by combining a signed ($\chi_{\mathrm{TT}}$) with the $\chi$–free quantity ($\Delta := M_4/3-\mathbb{E}[u_x^4]$) (see `kappa.py`). The quantity ($M_4$) itself is computed via **stabilized Bessel–Laplace integrals** with a $\sqrt{X}$ tail extrapolation (see `m4.py`).
...
- `m4.py` — **Exact Bessel–Laplace** evaluation of $M_4$ with identical kernel and
  $\sqrt{X}$ tail model; prints round-by-round cutoffs and the width of recent $M_4$
  values; emits tail-span stats in JSON.

- `kappa.py` — **TT baseline certificate** for $\kappa_\infty$ using $\chi_{\mathrm{TT}}$
  and the $\chi$–free $\Delta$ (or `M4` + `E[u^4]`), with optional propagation of a supplied
  $|\delta \Delta|$ into $|\delta \kappa|$.

- `h4.py` — **Exact $S^2$ moments** for the cubic $A_{1g}$ quartic harmonic
  $H_4$: $\langle H_4^2\rangle$, $\langle H_4^3\rangle$, $\langle H_4^4\rangle$
  (Dirichlet moments; rationals + high-precision echoes).

- `cq.py` — **York–quartic constant**:
  Monte‑Carlo certificate for $C_{\hat Q}$ constancy and the exact canonical
  $C_Q = 11/780$.

- `chitt.py` — **Linear-response scale** $\chi_{\mathrm{TT}} = 3\,C_Q$ (exact-fraction aware).

- `j2.py` — **Quarter-hop Jacobian$^2$** certificate at mid-edge: emits `J2 = 4/3`
...
- `zdir.py` — **Directional share from internal split**:
  $\zeta_{\text{dir}} = 8/25$ and $\zeta_{\text{wedge}} = \zeta_{\text{dir}}/N$ (K‑independent).
...
- `dL2.py` — **Quadratic correction**  
  $\Delta L^{(2)} = \tfrac12 f''(x_q)\,\langle H_4^2\rangle\,(\chi_{\mathrm{TT}}\Gamma)^2$.
...
> python run.py alpha.py
> ```
> A convenience wrapper is also provided: `run_all.cmd`.
...
- `m4.py` → computes $M_4$ $\to$ tail-span  
- `kappa.py` → combines $\chi_{\mathrm{TT}}$ and $\Delta$ to get $\kappa_\infty$  
